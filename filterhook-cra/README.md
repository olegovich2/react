...
['california', 'everything', 'aboveboard', 'washington', 'basketball', 'weathering', 'characters', 'literature', 
'contraband', 'appreciate']
---------------------------
Поведение должно быть аналогично описанному для домашнего задания Filter, только:
использовать функциональные компоненты и хуки;
разложить функционал на 3 компонента — 
в Controls птичка сортировки, строка фильтрации и кнопка сброса, 
в List результирующий список строк, 
и Filter объединяет это всё;
логика фильтрации и сортировки строк должна быть именно в компоненте Filter.


СОЗДАНИЕ НОВОГО ПРОЕКТА ---- npx create-react-app {filterhook-cra} - ИМЯ НОВОЙ ПАПКИ
